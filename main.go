...
import (
	...
	"github.com/yonisaka/loan-service/cmd"
	...
	"github.com/yonisaka/loan-service/rest"
	"github.com/yonisaka/loan-service/rest/route"
)
...
func main() {
	...
	grpcUserService, errClient := client.NewGRPCUserService(conf)
	if errClient != nil {
		log.Fatalf("grpc client unable connect to server, %v", errClient)
	}

	grpcBookService, errClient := client.NewGRPCBookService(conf)
	if errClient != nil {
		log.Fatalf("grpc client unable connect to server, %v", errClient)
	}

	grpcClient := client.NewGRPCClient(grpcUserService, grpcBookService)
